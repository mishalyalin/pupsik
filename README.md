...
- **`tools/doctor.py` health-check + safe auto-fix.** 14 deterministic checks across the workspace. Broken symlinks, stale lock files, ChromaDB orphan rows, oversized CLAUDE.md, dangling memory pointers. `check` is read-only; `fix-safe` only does safe repairs (never rewrites my prose); `orphans` lists unlinked entities for me to review. Cron-safe.
...
- **Auto-compact hooks.** `PreCompact` saves session state to disk before Claude compacts. `PostCompact` reminds it to restore. There's also a threshold env var that can fire compaction earlier than the default, but read the caveats in `docs/COMPACT_SETUP.md` first - the channel that actually reaches the agent differs per surface, and a threshold set above the model's real window does nothing at all.
- **Context budget, measured not guessed.** `tools/context_budget.py all` answers three questions from your own transcripts: at how many tokens does auto-compaction ACTUALLY fire on each model you use (`compactMetadata.preTokens`, so it's the agent's own number), how many tokens a session costs before you type a word, and how much of that is the part you can edit. Working room is the difference. This is the tool that explains "compacted three times in a row, then context window is full" - and it's why the size rule now caps `CLAUDE.md` by tokens instead of by lines, after a real one hit 244k characters (~93k tokens) in 301 lines and sailed under a 600-line cap.
- **MCP profiles, because instructions load unconditionally.** Tool *schemas* load on demand, but every configured MCP server's *instructions block* is injected at session start and cannot be unloaded mid-session - on a big stack that's tens of thousands of tokens before your first word. `tools/mcp_profile.py light` parks the servers you're not using today into `~/.claude/mcp-parked.json`, `full` restores them verbatim, `status` shows both. The park file is what makes the round-trip lossless - entries go back verbatim; non-MCP keys in your config files are never touched, and every rewrite also leaves a numbered `.bak-mcpprofile-<n>` beside the file as a human undo net. Takes effect on the next Claude start.
...
1. **`tools/doctor.py`** - 14 deterministic health checks across `check` / `fix-safe` / `orphans`. Cron-safe, never rewrites prose. Adapted from gbrain (Garry Tan, MIT).
